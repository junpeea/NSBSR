...
for(nd in 1:ND){
  setwd("C:/Users/user/Desktop/NSBSR_FINAL/THESIS_Results(New)/Result2")
  load(file=paste0("Matern05_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".Rdata"))
  Pred.result = summary(mcmc.list(NSBSR.PredMat))
  
  Yhat = simdata$Y
  Yhat = temp$statistics[1:simdata$nloc,1] + temp$statistics[(simdata$nloc+1):(2*simdata$nloc),1]
  
  pdf(paste0(Sys.Date(),"True_","Matern05_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".pdf"),width=7*1,height=7*1)
  par(mfrow=c(1,1),mar=c(1,1,1,1))
  fields::image.plot(matrix(Y,   sqrt(simdata$nloc),sqrt(simdata$nloc)),axes=F,cex.main=2.5,legend.mar=3.2,zlim=c(-2.1,2.1))
  dev.off()
  
  pdf(paste0(Sys.Date(),"Pred_","Matern05_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".pdf"),width=7*1,height=7*1)
  par(mfrow=c(1,1),mar=c(1,1,1,1))
