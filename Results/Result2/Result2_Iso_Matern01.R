...
for(nd in 1:ND){
  setwd("C:/Users/user/Desktop/NSBSR_FINAL/THESIS_Results(New)/Result2")
  load(file=paste0("IsoMatern01_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".Rdata"))
  ...
  pdf(paste0(Sys.Date(),"True_","IsoMatern01_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".pdf"),width=7*1,height=7*1)
  par(mfrow=c(1,1),mar=c(1,1,1,1))
  fields::image.plot(matrix(Y,   sqrt(simdata$nloc),sqrt(simdata$nloc)),axes=F,cex.main=2.5,legend.mar=3.2,zlim=c(-3.2,3.6))
  dev.off()
  
  pdf(paste0(Sys.Date(),"Pred_","IsoMatern01_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".pdf"),width=7*1,height=7*1)
  par(mfrow=c(1,1),mar=c(1,1,1,1))
  fields::image.plot(matrix(Yhat,sqrt(simdata$nloc),sqrt(simdata$nloc)),axes=F,cex.main=2.5,legend.mar=3.2,zlim=c(-3.2,3.6))
  dev.off()
