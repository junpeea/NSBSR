...
for(nd in 1:ND){
  setwd("D:/NSBSR_FINAL/THESIS_Results(New)/Result2")
  load(file=paste0("AnIsoMatern05_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".Rdata"))
  ...
  pdf(paste0(Sys.Date(),"True_","AnIsoMatern05_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".pdf"),width=7*1,height=7*1)
  par(mfrow=c(1,1),mar=c(1,1,1,1))
  fields::image.plot(matrix(Y,   sqrt(simdata$nloc),sqrt(simdata$nloc)),axes=F,cex.main=2.5,legend.mar=3.2,zlim=c(-1.8,2.7))
  dev.off()
  
  pdf(paste0(Sys.Date(),"Pred_","AnIsoMatern05_","Grid(",grid.size[1],",",grid.size[2],")_ND=",nd,".pdf"),width=7*1,height=7*1)
  par(mfrow=c(1,1),mar=c(1,1,1,1))
  fields::image.plot(matrix(Yhat,sqrt(simdata$nloc),sqrt(simdata$nloc)),axes=F,cex.main=2.5,legend.mar=3.2,zlim=c(-1.8,2.7))
  dev.off()
