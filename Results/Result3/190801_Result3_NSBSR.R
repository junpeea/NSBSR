...
for(type in 1:7){
  ...
  for(nd in 1:10){
    ...
    for(chain in 1:n.chain){
      ...
      for(iter in seq(5,n.iter,by=5)){
        
        if(iter%%250==0 | iter>n.burn){
        ...
        if(iter>n.burn & iter%%25==0){
          
          new.mcmcobj$EstiMat[((iter-n.burn)/25),] <- c(new.mcmcobj$Parset$beta,
                                                        new.mcmcobj$Parset$tau.e,
                                                        new.mcmcobj$Parset$theta,
                                                        new.mcmcobj$Parset$rho.th1,
                                                        new.mcmcobj$Parset$rho.th2)
          if((iter-n.burn)>25 & iter%%25==0){
            
            Esti.summary = summary(mcmc(new.mcmcobj$EstiMat[1:((iter-n.burn)/25),]))
            
            PredResult <- Predict_NSBSR(new.mcmcobj,Esti.summary,simdata)
            
            source("Suppl/Show_MCMC_NSBSR_Images.R")
            
            new.mcmcobj$PredMat[((iter-n.burn)/25),] <- c(as.vector(PredResult$result1),
    ...
    save.image(paste0("NSBSR_",dir[[type]],"_nd=",nd,".Rdata"))
