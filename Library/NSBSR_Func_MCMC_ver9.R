...
# Project name : NSBSR_func_MCMC_ver9
...
Update_Theta_NSBSR = function(mcmcobj,simdata){
  ...
  # newtheta[(mbar-15):mbar] <- newtheta[(mbar-15):mbar]-1
  
  # lambda: rescaled to 1
  tau.star  = solve(tau.th*solve(exp(-(1e-03)*freq.dist1-(1e-03)*freq.dist2))+diag(1/vlabel))
  if(!is.symmetric.matrix(tau.star)){
    temp <- t(tau.star)
    temp[lower.tri(tau.star)] <- tau.star[lower.tri(tau.star)]
    tau.star <- temp
  }
  nu.star  = tau.star%*%diag(1/vlabel)%*%(phi-klabel)
  seednum <- seednum + 10; set.seed(seednum)
  newlambda = exp(matrix(rmvnorm(1,mean=nu.star,sigma=tau.star),ncol=1))
  if(n%%2==0){newlambda = c(newlambda,rev(newlambda))
  }else{newlambda = c(newlambda,rev(newlambda[1:(length(newlambda)-1)]))} 
  
  # output
  # new.mcmcobj$Parset$lambda <- newlambda
  new.mcmcobj$Parset$theta  <- newtheta
  new.mcmcobj$Parset$lambda  <- c(exp(newtheta),rev(exp(newtheta)))
  new.mcmcobj$seed   <- seednum
...
Update_Theta_prior_NSBSR = function(mcmcobj,simdata,rho.list,c=1,d=1){
  ...
  # Update nu.th
  g <- 1
  nu.sig2.star  = solve(chol2inv(chol(tau.mm))+diag(g,mbar))
  if(!is.symmetric.matrix(nu.sig2.star)){
    temp <- t(nu.sig2.star)
    temp[lower.tri(nu.sig2.star)] <- nu.sig2.star[lower.tri(nu.sig2.star)]
    nu.sig2.star <- temp
  }
  nu.mu.star  = nu.sig2.star%*%diag(g,mbar)%*%nu.th
  seednum <- seednum + 10; set.seed(seednum)
  if(is.positive.definite(nu.sig2.star)){
    new.nu.th = matrix(rmvnorm(1,mean= nu.mu.star,sigma=nu.sig2.star),ncol=1)
  }else{
    cat("positive definite failed","\n")
    new.nu.th = matrix(rmvnorm(1,mean= nu.mu.star,sigma=diag(g,mbar)),ncol=1)
  }
  ...
  # output
  new.mcmcobj$Parset$nu.th   <- new.nu.th
