...
Update_beta_sig_phi_NSBSR = function(mcmcobj,simdata,beta.mu0=0,beta.sig20=100,a=100,b=100){
  ...
  comp0 = Re(fftw2d(matrix(cor_mtx[,1],17,17)))
  comp = apply(cbind(Xobs,Yobs),2,function(w){
    fft1 = fftw2d(matrix(w,17,17))
    D = diag(1/(comp0%>%as.vector))
    fft2 = D%*%matrix(fft1,ncol=1)
    fft3 = Re(fftw2d(matrix(fft2,17,17),inverse=TRUE)) %>% as.vector
    fft3/289
  })
  comp1 = comp[,1:3]; comp2 = comp[,4]
  # cor_mtx_inv = Re(Effsolve(esti_cor_block,n))
  # comp1 = Re(cor_mtx_inv)%*%Xobs
  # comp2 = Re(cor_mtx_inv)%*%Yobs
  ...
  fft1 = fftw2d(matrix(sc.resi,17,17))
  D = diag(1/(comp0%>%as.vector))
  fft2 = D%*%matrix(fft1,ncol=1)
  fft3 = Re(fftw2d(matrix(fft2,17,17),inverse=TRUE)) %>% as.vector
  comp3 = fft3/289
...
Predict_NSBSR = function(mcmcobj,Esti.summary,simdata){
  ...
  temp = toeplitz.spam(c(1:sqrt(nrow(data0))))%>%as.matrix
  temp2 = lapply(c(1:sqrt(nrow(data0))),function(w) toeplitz.spam(pred_corr[,w]))
  temp5 = kronecker(temp,matrix(1,sqrt(nrow(data0)),sqrt(nrow(data0))))
  pred_cor_mtx = matrix(0,nrow(data0),nrow(data0))
  for(i in 1:sqrt(nrow(data0))){
    
    temp6 = matrix(as.numeric(temp5 %in% i),nrow=,nrow(data0))
    temp7 = kronecker(matrix(1,sqrt(nrow(data0)),sqrt(nrow(data0))),temp2[[i]] %>% as.matrix)
